(function() {
 //   var sel = selectionObj = window.getSelection();
//    var rangeObj = selectionObj.getRangeAt(0);
//    var docFragment = rangeObj.cloneContents();
//    var tempDiv = document.createElement("div");
//    tempDiv.appendChild(docFragment);
//    html = tempDiv.innerHTML;
//	html.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/gi, function (match, img) {
//    if(!(img.substring(0,4)=='http' || img.substring(0,2)=='//')){
//		var lianjie=''
//		if(!img.substring(0,1)=='/'){
//			lianjie='/'
//		}
//		matchtwo=match.replace(img,'http://'+window.location.host+lianjie+img);
//		html=html.replace(match,matchtwo);
//	}
//});
        ...
        for(var s=0, sl=siblingNodes.length; s < sl; s+=1) {
            ...
            **/
            ...
            if(append) {
				...
				var childs = nodeToAppend.childNodes;
				for(var i = 0; i < childs.length; i++) { 
				articleContent.appendChild(childs[i]);
} */
                articleContent.appendChild(nodeToAppend);
				paras = nodeToAppend.getElementsByClassName('node-tag');
for(i=0;i<paras.length;i++){
     //删除元素 元素.parentNode.removeChild(元素);
    if (paras[i] != null) 
          paras[i].parentNode.removeChild( paras[i]); 
}
				//nodeToAppend.removeChild(nodeToAppend.getElementsByClassName('node-tag')[0]);//美文网标签
				if(nodeToAppend.innerHTML.length < 200){
					continue
				}
				htmlcode+=nodeToAppend.innerHTML;
				break;
            }
        }
        /**
    ...
    **/
    ...
    function getArticleTitle() {
        var curTitle = "",
            origTitle = "";
        if (curTitles=document.getElementsByTagName('h1')){
			//alert(curTitle.innerText);
			for(i=0;i<curTitles.length;i++){
				if(document.title.substring(0,2)==curTitles[i].innerText.substring(0,2)){
					//alert(curTitles[i].innerText);
					return curTitles[i].innerText;
				}
			}
		}else if(curTitle=document.getElementsByTagName('h2')[0]){
			return curTitle.innerText;
		}
        try {
            curTitle = origTitle = document.title;
        }
...
function isVisibleNode(a) {
		if (a.nodeType) {
			if (a.nodeType == 3) {
				return true
			}
			if (isBrowser() == 'IE') {
				if (a.currentStyle != null && a.currentStyle['display'] == "none") {
					return false
				}
			} else {
				try {
					if (window.getComputedStyle(a, null)['display'] == "none") {
						return false
					}
				} catch (e) {
					return false
...
function dom(b) {
		...
		for (var i = 0; i < c.length; i++) {
			var d = c[i];
			var e = d.attributes;
			if (d.nodeName.toLowerCase() == "script" || d.nodeName.toLowerCase() == "iframe" || d.nodeName.toLowerCase() == "link" || d.nodeName.toLowerCase() == "meta" || !isVisibleNode(d)) {
				d.parentNode.removeChild(d)
			} else if (d.nodeName.toLowerCase() == "a" || d.nodeName.toLowerCase() == "img" || d.nodeName.toLowerCase() == "embed") {
				if (e != null) {
					for (var j = 0; j < e.length; j++) {
						var a = e[j].nodeName.toLowerCase();
						var v = e[j].nodeValue;
						if (a == "href" || a == "src") {
...
function GetSelectHtml() {
		...
		var d;
		if (isBrowser() == 'IE') {
			if (document.selection.type == "None" || document.selection.type == "none") return "";
			c = document.selection.createRange();
			a = document.createElement("span");
			a.innerHTML = c.htmlText
		} else {
			b = window.getSelection();
			if (b.rangeCount == 0) return "";
			c = b.getRangeAt(0);
			var e = c.startContainer;
			a = e.ownerDocument.createElement("span");
    ...
    post('http://localhost/editor/index.php', {
        content1: html,title:title
    });
